...
mysql -u$PRO_USERID --port $PRO_PORT  -p$PRO_PASSWORD -c -h $PROD_IPADDRESS -e "select trim(script_name) from $versioning_db.$versioning_table where project_name=$ORGANIZATION and model_name=$APPLICATION order by script_date;"
