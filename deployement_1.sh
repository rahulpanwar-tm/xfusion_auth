...
Array2=($(mysql -u$GLOB_USERID --port $GLOB_PORT  -p$GLOB_PASSWORD -c -h $GLOB_IPADDRESS   -Bse "select trim(script_name) from xfusion_config.version_status where project_name=$ORGANIZATION order by script_date;"))
